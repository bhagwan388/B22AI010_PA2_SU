...
The codebase now includes renamed package paths for a cleaner structure:

- `speech_pipeline.runtime` (was `speech_pipeline.pipeline`)
- `speech_pipeline.benchmark` (was `speech_pipeline.evaluate`)
- `speech_pipeline.settings` (was `speech_pipeline.config`)
- `speech_pipeline.recognition` (was `speech_pipeline.asr`)
- `speech_pipeline.enhancement` (was `speech_pipeline.denoise`)
- `speech_pipeline.phonemization` (was `speech_pipeline.g2p`)
- `speech_pipeline.language_id` (was `speech_pipeline.lid`)
- `speech_pipeline.translation_engine` (was `speech_pipeline.translation`)
- `speech_pipeline.synthesis` (was `speech_pipeline.tts`)
- `speech_pipeline.spoofing` (was `speech_pipeline.antispoof`)
- `speech_pipeline.prosody_control` (was `speech_pipeline.prosody`)
- `speech_pipeline.speaker_id` (was `speech_pipeline.speaker`)
- `speech_pipeline.common` (was `speech_pipeline.utils`)

Legacy paths are preserved as compatibility layers.

## Run the full pipeline

```bash
python -m speech_pipeline.apps.pipeline_app \
...
```bash
python -m speech_pipeline.recognition.corpus_builder \
...
```bash
python -m speech_pipeline.language_id.trainer \
...
```bash
python -m speech_pipeline.language_id.preprocessor \
...
```bash
python -m speech_pipeline.language_id.eval_segment_prep \
...
```bash
python -m speech_pipeline.language_id.evaluator \
...
```bash
python -m speech_pipeline.spoofing.manifest_builder \
...
```bash
python -m speech_pipeline.spoofing.trainer \
...
```bash
python -m speech_pipeline.apps.evaluation_app \
