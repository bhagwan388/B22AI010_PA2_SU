...
from .spoofing.inference import AntiSpoofInferencer
from .recognition.whisper_decoder import ConstrainedWhisperASR
from .language_id.inference import LIDInferencer
from .common.audio_ops import load_audio, estimate_snr_db
...
def score_asr_manifest(manifest_path: str | Path, model_name: str, lm_corpus: str | Path | None, device: str | None) -> ASRMetrics:
...
def score_lid_switches(manifest_path: str | Path, lid_checkpoint: str | Path, device: str | None) -> LIDSwitchMetrics:
...
def score_mcd_manifest(manifest_path: str | Path, sample_rate: int = 16_000) -> MCDMetrics:
...
def score_attack_manifest(manifest_path: str | Path, lid_checkpoint: str | Path, device: str | None) -> AttackMetrics:
...
def compose_report(
    ...
    if asr_manifest is not None:
        report["asr"] = asdict(score_asr_manifest(asr_manifest, whisper_model, lm_corpus, device))
    if lid_manifest is not None:
        if lid_checkpoint is None:
            raise ValueError("--lid-checkpoint is required for LID evaluation")
        report["lid_switch"] = asdict(score_lid_switches(lid_manifest, lid_checkpoint, device))
    if mcd_manifest is not None:
        report["mcd"] = asdict(score_mcd_manifest(mcd_manifest))
    ...
    if attack_manifest is not None:
        ...
        report["attack"] = asdict(score_attack_manifest(attack_manifest, lid_checkpoint, device))
    return report
...
evaluate_asr = score_asr_manifest
evaluate_lid_switches = score_lid_switches
evaluate_mcd = score_mcd_manifest
evaluate_spoof = score_spoof_manifest
estimate_min_epsilon_to_flip_lid = estimate_min_flip_epsilon
evaluate_attack = score_attack_manifest
